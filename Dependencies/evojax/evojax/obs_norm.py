...
class ObsNormalizer(object):
    ...
    def __init__(self,
        ...
        print(obs_shape)
        self._obs_dim = obs_shape[-1] - task_info_holder
        self._task_dim = task_info_holder
        if task_info_holder != 0:
            self._obs_shape = (*obs_shape[:-1], self._obs_dim)
        else:
            self._obs_shape = obs_shape
        self._obs_size = np.prod(self._obs_shape)
    ...
    def normalize_obs(self,
        ...
        if self.is_dummy:
            ...
            # if self._task_info_holder:
            task_info = obs[:, self._obs_dim:]
            obs = obs[:, :self._obs_dim]
            print(task_info.shape, obs.shape) 
            normed_obs = normalize(
                obs=obs,
            ...
            return jnp.concatenate([normed_obs, task_info], axis=-1)
            # else:
            #     return normalize(
            #         obs=obs,
            #         obs_params=obs_params,
    ...
    def update_normalization_params(self,
        ...
        if self.is_dummy:
            ...
            obs_buffer = obs_buffer[:, :, :self._obs_dim]
            # obs_mask = obs_mask[:, :, :-self._task_info_holder]
