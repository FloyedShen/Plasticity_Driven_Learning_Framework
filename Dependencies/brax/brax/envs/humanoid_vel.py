    ...
    * v0: Initial versions release (1.0.0)
    ...
    def reset(self, rng: jp.ndarray) -> env.State:
        ...
        if self._target_vel is not None:
            target_vel = jp.array(self._target_vel)
        else:
            target_vel = \
                (rng[0] % self._num_tasks) * (self.task_max - self.task_min) / self._num_tasks + self.task_min
            # target_vel = jax.random.uniform(rng3, minval=self.task_min, maxval=self.task_max)
        obs = self._get_obs(qp, self.sys.info(qp), jp.zeros(self.action_size), target_vel)
    ...
    def step(self, state: env.State, action: jp.ndarray) -> env.State:
        ...
        target_vel = state.obs[-1]
        forward_reward = -self._forward_reward_weight * abs(velocity[0] - target_vel)
        ...
        obs = self._get_obs(qp, info, action, target_vel)
    ...
    def _get_obs(self, qp: brax.QP, info: brax.Info,
                 action: jp.ndarray, target_vel) -> jp.ndarray:
        ...
        return jp.concatenate(qpos + qvel + cinert + cvel + qfrc_actuator + [jp.array([target_vel, ])])
