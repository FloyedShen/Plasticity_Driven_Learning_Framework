...
# Datetime  : 2023/2/20 21:53
# User      : yu
# Product   : PyCharm
# Project   : brax
# File      : humanoid_dir.py
# explain   :
...
class HumanoidDir(env.Env):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def reset(self, rng: jp.ndarray) -> env.State:
        ...
        _, rng1, rng2 = jp.random_split(rng, 3)
        ...
        if self._target_direction is not None:
            target_direction = jp.array(self._target_direction)
        else:
            target_direction = \
                (rng[0] % self._num_tasks) * (self.task_max - self.task_min) / self._num_tasks + self.task_min

        obs = self._get_obs(qp, self.sys.info(qp), jp.zeros(self.action_size), target_direction)
    ...
    def step(self, state: env.State, action: jp.ndarray) -> env.State:
        ...
        direction = state.obs[-1]
        _cos = jp.cos(direction)
        _sin = jp.sin(direction)
        forward_reward = velocity[0] * _cos + velocity[1] * _sin
        ...
        obs = self._get_obs(qp, info, action, direction)
    ...
    def _get_obs(self, qp: brax.QP, info: brax.Info,
                 action: jp.ndarray, target_direction) -> jp.ndarray:
        ...
        return jp.concatenate(qpos + qvel + cinert + cvel + qfrc_actuator + [jp.array([target_direction, ])])
