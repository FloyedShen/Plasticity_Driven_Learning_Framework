...
class Hopperv(env.Env):
  ...
  * v0: Initial versions release (1.0.0)
  ...
  def reset(self, rng: jp.ndarray) -> env.State:
    ...
    return env.State(qp, obs[5:11], reward, done, metrics)
  ...
  def step(self, state: env.State, action: jp.ndarray) -> env.State:
    ...
    return state.replace(qp=qp, obs=obs[5:11], reward=reward, done=done)
